-- Phase 2 Database Setup - Minimal Version
-- Fix all type issues
...
DROP POLICY IF EXISTS "Users can view their own registration logs" ON user_registration_logs;
DROP POLICY IF EXISTS "Admins can view all registration logs" ON user_registration_logs;
DROP POLICY IF EXISTS "Admins can manage approval queue" ON user_approval_queue;
...
-- 6. Create function to log user actions
...
-- 7. Create function to approve user
...
-- 8. Create function to reject user
...
-- 10. Update existing admin user permissions to JSONB format
UPDATE user_profiles 
SET permissions = '["admin:all"]'::jsonb 
WHERE role = 'admin' AND permissions IS NULL;

-- 11. Update existing users to have proper status
UPDATE user_profiles 
