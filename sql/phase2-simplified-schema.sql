-- Simplified Phase 2 Database Setup
-- Fix for JSONB permissions column
...
-- 8. Create function to approve user (with JSONB permissions)
...
-- 13. Update existing admin user permissions to JSONB format
UPDATE user_profiles 
SET permissions = '["admin:all"]'::jsonb 
WHERE role = 'admin' AND permissions IS NULL;

-- 14. Update existing users to have proper status
UPDATE user_profiles 
