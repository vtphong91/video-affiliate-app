...
export function ScheduleCard({ schedule, onDelete, onRetry }: ScheduleCardProps) {
  ...
  const formatScheduledTime = (timestamp: string) => {
    const scheduledUtc = new Date(timestamp); // This is already UTC from DB
    const nowUtc = new Date(); // Current time in UTC
    
    const diffMs = scheduledUtc.getTime() - nowUtc.getTime();
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-gray-600">
              {new Date(schedule.scheduled_for).toLocaleString('vi-VN', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
                minute: '2-digit',
                timeZone: 'Asia/Ho_Chi_Minh', // FIX: Specify timezone
              })}
            </span>
            {schedule.status === 'pending' && (
          ...
          {schedule.posted_at && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-gray-600">
                Đã đăng: {new Date(schedule.posted_at).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}
              </span>
            </div>
          )}
